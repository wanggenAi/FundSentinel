...
- `SseMarketCalendarProvider`: implemented official Shanghai Stock Exchange market-calendar provider. It uses the SSE page-backed query endpoints for shareholder meetings, IPO calendar items, and roadshow events as market-event context only.
...
- `https://www.sse.com.cn/disclosure/dealinstruc/calendar/index.shtml` plus the backed `https://query.sse.com.cn/commonSoaQuery.do` / `commonQuery.do` endpoints for official SSE market-calendar events. Argus treats these as market context only.
