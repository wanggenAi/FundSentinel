...
- `AmacPublicFundDataProvider`: implemented official AMAC public-fund market-data provider. It parses monthly public-fund market-data PDF references as industry baseline evidence only; it does not provide fund NAV, holdings, report bodies, trading access, or advice.
...
- `FundCompanyReportProvider`: implemented official report evidence coordinator. It consumes verified official periodic-report PDF metadata already collected by concrete fund-company/regulatory providers and fails explicitly when no such evidence exists.
...
- `MiitOfficialProvider`: implemented official Ministry of Industry and Information Technology provider. It parses MIIT homepage policy-file, policy-interpretation, industry-data, industry-news, and consultation/publication lists for manufacturing, digital-economy, telecom, software, AI, and new-energy-vehicle context only.
- `SseMarketCalendarProvider`: implemented official Shanghai/Shenzhen exchange market-event provider. It uses the SSE page-backed query endpoints for shareholder meetings, IPO calendar items, and roadshow events, plus SZSE official news/notice pages, as market-event context only.
...
- `MofFiscalProvider`: implemented official Ministry of Finance fiscal revenue/expenditure provider. It parses MOF monthly national fiscal revenue and expenditure releases for bond-fund/macro context and never treats fiscal data as fund NAV or advice.
- `SafeMacroProvider`: implemented official State Administration of Foreign Exchange reserve-assets provider. It parses SAFE monthly official reserve-asset HTML tables for USD-denominated cross-border/QDII context and never treats macro data as fund NAV or advice.
- `PbcMacroProvider`: implemented official People's Bank of China money-supply provider. It parses PBC public monthly Money Supply HTML tables for M2/M1/M0 liquidity context and never treats macro data as fund NAV or advice.
...
- `OecdCliProvider`: implemented official OECD SDMX provider for monthly Composite Leading Indicator rows for configured countries. It provides global/QDII macro context only and does not provide fund NAV, holdings, reports, or advice.
...
- `SecEdgarProvider`: implemented official SEC EDGAR submissions provider for US ticker/CIK inputs. It records recent NPORT-P, N-CEN, N-CSR, N-1A/485BPOS/497 filing metadata and URLs for QDII/global ETF context only; it does not parse filing body text or provide NAV/advice.
- `HkexOfficialProvider`: implemented official HKEXnews listed-company announcement provider for Hong Kong/QDII/H-share context. It resolves HK stock candidates through HKEX's own stock prefix endpoint, parses title-search result metadata, and never treats issuer announcements as fund NAV, fund reports, trading signals, or advice.
...
- `https://www.amac.org.cn/sjtj/tjbg/gmjj/` for official AMAC monthly public-fund market-data PDF references as industry baseline context only.
...
- `https://www.miit.gov.cn/index.html` for official Ministry of Industry and Information Technology homepage policy, policy-interpretation, industry-data, and industry-news lists. Argus treats this as manufacturing/digital-economy context only.
- `https://www.sse.com.cn/disclosure/dealinstruc/calendar/index.shtml` plus the backed `https://query.sse.com.cn/commonSoaQuery.do` / `commonQuery.do` endpoints for official SSE market-calendar events, and `https://www.szse.cn/aboutus/trends/news/` / `https://www.szse.cn/disclosure/notice/general/` for official SZSE news/notice items. Argus treats these as market context only.
- `https://www.mof.gov.cn/zhengwuxinxi/redianzhuanti/quanguocaizhengshouzhiqingkuang/` plus the linked fiscal revenue/expenditure article for official MOF general public budget, tax, non-tax, government-fund, land-transfer, and debt-interest indicators.
- `https://www.safe.gov.cn/safe/gfcbzc/index.html` plus the linked annual official reserve-assets HTML table for SAFE foreign currency reserves, IMF reserve position, SDRs, gold reserves, other reserves, and total reserve assets in USD units.
- `https://www.pbc.gov.cn/diaochatongjisi/116219/116319/5570903/5570886/index.html` plus the linked Money Supply HTML table for official PBC M2/M1/M0 liquidity indicators.
- `https://www.sec.gov/files/company_tickers_exchange.json` plus `https://data.sec.gov/submissions/CIK{cik}.json` for official SEC EDGAR ticker/CIK resolution and fund filing metadata. This provider only runs for US ticker/CIK-style inputs, not Chinese 6-digit fund codes.
- `https://www1.hkexnews.hk/search/prefix.do` plus `https://www1.hkexnews.hk/search/titlesearch.xhtml` for official HKEXnews stock lookup and listed-company announcement metadata. This provider only runs for configured HKEX stock candidates or acquired Hong Kong/QDII holding context, and it does not clear `official_fund_reports`.
...
- `https://sdmx.oecd.org/public/rest/data/OECD.SDD.STES,DSD_STES@DF_CLI` for official OECD monthly Composite Leading Indicator SDMX CSV rows for configured countries. Argus treats this as macro context only.
